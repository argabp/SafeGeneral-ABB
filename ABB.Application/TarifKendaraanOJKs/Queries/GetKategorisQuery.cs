    ...
    public class GetKategorisQueryHandler : IRequestHandler<GetKategorisQuery, List<KategoriDto>>
    {
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                return (await _connectionFactory.Query<KategoriDto>(@"SELECT RTRIM(LTRIM(kd_kategori)) kd_kategori, RTRIM(LTRIM(nm_kategori)) nm_kategori FROM v_rf10_01")).ToList();
            }
            catch (Exception ex)
            {
