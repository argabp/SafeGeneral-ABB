using System;
using System.Threading;
using System.Threading.Tasks;
using ABB.Application.Common.Interfaces;
using MediatR;
using TipeAkun104Entity = ABB.Domain.Entities.TipeAkun104;


namespace ABB.Application.TipeAkuns104.Commands
{
    // Ini adalah "surat perintah" yang sudah Anda buat
    public class CreateTipeAkun104Command : IRequest<string>
    {
        public string Kode { get; set; }
        public string NamaTipe { get; set; }
        public string Pos { get; set; }
        public bool? Kasbank { get; set; }
    }

    // ---> INI BAGIAN YANG HILANG: "Petugas Pelaksana" <---
    public class CreateTipeAkun104CommandHandler : IRequestHandler<CreateTipeAkun104Command, string>
    {
        private readonly IDbContextPstNota _context;

        public CreateTipeAkun104CommandHandler(IDbContextPstNota context)
        {
            _context = context;
        }

        public async Task<string> Handle(CreateTipeAkun104Command request, CancellationToken cancellationToken)
        {
            // 1. Buat objek Entity dari data di dalam Command
            var entity = new TipeAkun104Entity
            {
                Kode = request.Kode,
                NamaTipe = request.NamaTipe,
                Pos = request.Pos,
                Kasbank = false

            };

            // 2. Tambahkan entity baru ke DbContext
            _context.TipeAkun104.Add(entity);

            // 3. Simpan perubahan ke database
            await _context.SaveChangesAsync(cancellationToken);

            // 4. Kembalikan Primary Key dari data yang baru dibuat
            return entity.Kode;
        }
    }
}