...
using ABB.Application.EditJurnals117.Queries;

namespace ABB.Application.EditJurnals117.Commands
{
    public class UpdateJurnal117Command : IRequest<bool>
    {
        ...
        // Data Detail (Grid)
        public List<DetailJurnal117Dto> Details { get; set; }
    }

    public class UpdateJurnal117CommandHandler : IRequestHandler<UpdateJurnal117Command, bool>
    {
        private readonly IDbContextPstNota _context;

        public UpdateJurnal117CommandHandler(IDbContextPstNota context)
        {
            _context = context;
        }

        public async Task<bool> Handle(UpdateJurnal117Command request, CancellationToken cancellationToken)
        {
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync(cancellationToken))
            {
                try
                {
                    ...
                    await _context.Database.ExecuteSqlRawAsync(
                        "DELETE FROM abb_jurnal117 WHERE gl_bukti = {0}", 
                        request.NoBukti
                    );
                    ...
                    {
                        await _context.Database.ExecuteSqlRawAsync(
                            "EXEC sp_UpdateEditJurnal117 @p0, @p1, @p2, @p3, @p4, @p5, @p6, @p7, @p8, @p9, @p10, @p11, @p12",
                            request.KodeLokasi,
                            request.NoBukti,
                            request.TglBukti,
                            item.NoNota ?? "",
