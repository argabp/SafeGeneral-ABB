...
{
    public static class DetailKontrakTreatyKeluarXOLConfig
    {
        public static GridConfig Create()
        {
            return new GridConfig
            {
                FromSql = @"
                            FROM (
                                SELECT 
                                    RTRIM(p.kd_cb) + RTRIM(p.kd_jns_sor) + RTRIM(p.kd_tty_npps) + RTRIM(p.kd_grp_pas) + RTRIM(p.kd_rk_pas) AS Id,
                                    p.*,
                ...
                BaseWhere = "(src.kd_cb = @kd_cb AND src.kd_jns_sor = @kd_jns_sor AND src.kd_tty_npps = @kd_tty_npps)",
