...
namespace ABB.Application.RegisterKlaims.Queries
{
    ...
    public class GetAkseptasisQueryHandler : IRequestHandler<GetAkseptasisQuery, List<AkseptasiDto>>
    {
        ...
        public async Task<List<AkseptasiDto>> Handle(GetAkseptasisQuery request, CancellationToken cancellationToken)
        {
            ...
            return (await _connectionFactory.Query<AkseptasiDto>(@"SELECT TOP 100 p.*, cb.nm_cb, cob.nm_cob, scob.nm_scob
				FROM v_uw04e p
					INNER JOIN rf01 cb
						ON p.kd_cb = cb.kd_cb
					INNER JOIN rf04 cob
				...
				WHERE cb.kd_cb = @KodeCabang AND p.kd_cob = @kd_cob AND p.kd_scob = @kd_scob AND (cb.nm_cb like '%'+@SearchKeyword+'%' 
