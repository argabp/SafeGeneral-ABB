...
{
    public class GetCetakNotaKlaimReasuransiQuery : IRequest<string>
    {
    ...
    public class GetCetakNotaKlaimReasuransiQueryHandler : IRequestHandler<GetCetakNotaKlaimReasuransiQuery, string>
    {
        private readonly IDbConnectionPst _connectionPst;
        private readonly IHostEnvironment _environment;
        private readonly ILogger<GetCetakNotaKlaimReasuransiQueryHandler> _logger;

        public GetCetakNotaKlaimReasuransiQueryHandler(IDbConnectionPst connectionPst, 
            IHostEnvironment environment, ILogger<GetCetakNotaKlaimReasuransiQueryHandler> logger)
        {
        ...
        public async Task<string> Handle(GetCetakNotaKlaimReasuransiQuery request, CancellationToken cancellationToken)
        {
            return await ExceptionHelper.ExecuteWithLoggingAsync(async () =>
            {
                var datas = (await _connectionPst.QueryProc<CetakNotaKlaimReasuransiDto>("spr_cl12r_01",
                    new
                    {
                ...
                var resultTemplate = templateProfileResult.Render(new
                {
                    ...
                    data.no_pol_ttg, data.kt_ttg, data.kd_mtu_symbol, data.kd_tl,
                    data.nm_kt_cb, data.tgl_nt_ind, data.no_nota, data.nm_scob, data.nm_scob_ing,
                    data.kd_cob, data.kd_scob, data.no_lpk,
                    nilai_share_bgu = ReportHelper.ConvertToReportFormat(data.nilai_share_bgu), data.ket_nt,
