...
function searchKeyword_OnKeyUp() {
    $('#SearchKeyword').keyup(function () {
        refreshGrid("#CetakNotaKomisiTambahanGrid");
    });
}
...
function viewReport(e){
    showProgressOnGrid('#CetakNotaKomisiTambahanGrid');
    ...
    ajaxPost("/CetakNotaKomisiTambahan/GenerateReport", JSON.stringify(form),
        function (response) {
            if(response.Status === "OK"){
                window.open("/Reports/" + response.Data + "/CetakNotaKomisiTambahan.pdf",  '_blank');
            } else {
                showMessage('Error', response.Message);
            }
            closeProgressOnGrid('#CetakNotaKomisiTambahanGrid');
        }, 
    );
}