...
using ABB.Application.KontrakTreatyMasukXOLs.Queries;
using ABB.Web.Modules.Base;
using ABB.Web.Modules.KontrakTreatyMasukXOL.Models;
using Microsoft.AspNetCore.Mvc;

namespace ABB.Web.Modules.KontrakTreatyMasukXOL
{
    public class KontrakTreatyMasukXOLController : AuthorizedBaseController
    {
        ...
        public async Task<ActionResult> GetKontrakTreatyMasukXOLs(GridRequest grid)
        {
            var result = await Mediator.Send(new GetKontrakTreatyMasukXOLsQuery()
            {
                Grid = grid
            });
        ...
        {
            return View(new KontrakTreatyMasukXOLViewModel()
            {
                kd_cb = "PS10"
            });
        }
        ...
        {
            var command = new GetKontrakTreatyMasukXOLQuery()
            {
                kd_cb = kd_cb,
            ...
            return View(Mapper.Map<KontrakTreatyMasukXOLViewModel>(result));
        }
        
        [HttpPost]
        public async Task<IActionResult> SaveKontrakTreatyMasukXOL([FromBody] KontrakTreatyMasukXOLViewModel model)
        {
            try
            {
                var command = Mapper.Map<SaveKontrakTreatyMasukXOLCommand>(model);
        ...
        [HttpGet]
        public async Task<IActionResult> DeleteKontrakTreatyMasukXOL(string kd_cb, string kd_jns_sor, string kd_tty_msk)
        {
            try
            {
                var command = new DeleteKontrakTreatyMasukXOLCommand()
                {
                    kd_cb = kd_cb, 
                    kd_jns_sor = kd_jns_sor, 
                    kd_tty_msk = kd_tty_msk
        ...
        public async Task<JsonResult> GetKeteranganTreaty(string kd_cob, string nm_jns_sor, decimal thn_uw)
        {
            try
            {
                var result = await Mediator.Send(new GetKeteranganTreatyXOLQuery()
                {
                    kd_cob = kd_cob,
