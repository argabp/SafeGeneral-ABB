...
function searchKeyword_OnKeyUp() {
    $('#SearchKeyword').keyup(function () {
        refreshGrid("#CancelPostingNotaPremiXOLKeluarGrid");
    });
}

function searchFilterCancelPostingNotaPremiXOLKeluar(e) {
...
function onChangeGridCancelPostingNotaPremiXOLKeluar(e) {
    var grid = e.sender;
...
function accountingPostingNotaPremiXOLKeluar(){
    showConfirmation('Confirmation', `Are you sure you want to Posting Accounting?`,
        function () {
            showProgressOnGrid('#CancelPostingNotaPremiXOLKeluarGrid');

            ajaxPost("/CancelPostingNotaPremiXOLKeluar/PostingAccounting", JSON.stringify(selectedRowsData),
                function (response) {
                    ...
                    var grid = $("#CancelPostingNotaPremiXOLKeluarGrid").data("kendoGrid");
                    if (grid) {
                        grid.clearSelection();
                    }
                    selectedRowsData = [];
                    refreshGrid('#CancelPostingNotaPremiXOLKeluarGrid');
                    closeProgressOnGrid('#CancelPostingNotaPremiXOLKeluarGrid');
                },
            );
        }
    );
...
function cancelPostingNotaPremiXOLKeluar(){
    showConfirmation('Confirmation', `Are you sure you want to cancel?`,
        function () {
            showProgressOnGrid('#CancelPostingNotaPremiXOLKeluarGrid');

            ajaxPost("/CancelPostingNotaPremiXOLKeluar/Cancel", JSON.stringify(selectedRowsData),
                function (response) {
                    if(response.Status === "OK"){
