...
function btnPreview_Click() {
    $('#btn-preview').click(function () {
        showProgressByElement($('#LaporanProduksiAsuransiKeluarForm'));
        setTimeout(function () {
            previewReport('/LaporanProduksiAsuransiKeluar/GenerateReport')
        }, 500);
    });
}

function previewReport(url) {
    var form = getFormData($('#LaporanProduksiAsuransiKeluarForm'));
    ...
    ajaxPost(url, data,
        function (response) {
            if(response.Status === "OK"){
                window.open("/Reports/" + response.Data + "/LaporanProduksiAsuransiKeluar.pdf",  '_blank');
            } else {
                showMessage('Error', response.Message);
            }

            closeProgressByElement($('#LaporanProduksiAsuransiKeluarForm'));
        }
    );
}
