...
function searchKeyword_OnKeyUp() {
    $('#SearchKeyword').keyup(function () {
        refreshGrid("#ProsesAlokasiKlaimReasuransiGrid");
    });
}

function searchFilterProsesAlokasiKlaimReasuransi(e) {
...
function onChangeGridProsesAlokasiKlaimReasuransi(e) {
    var grid = e.sender;
...
function prosesAlokasiKlaimReasuransi(){
    showConfirmation('Confirmation', `Are you sure you want to Auto SOL?`,
        function () {
            showProgressOnGrid('#ProsesAlokasiKlaimReasuransiGrid');

            ajaxPost("/ProsesAlokasiKlaimReasuransi/AlokasiReasuransi", JSON.stringify(selectedRowsData),
                function (response) {
                    if(response.Result === "OK"){
                        showMessage("Success", "Proses Alokasi Reasuransi Selesai!")
                    } else {
                        showMessage('Error', response.Message);
                    }
                    var grid = $("#ProsesAlokasiKlaimReasuransiGrid").data("kendoGrid");
                    if (grid) {
                        grid.clearSelection();
                    }
                    selectedRowsData = [];
                    refreshGrid('#ProsesAlokasiKlaimReasuransiGrid');
                    closeProgressOnGrid('#ProsesAlokasiKlaimReasuransiGrid');
                },
            );
        }
    );
