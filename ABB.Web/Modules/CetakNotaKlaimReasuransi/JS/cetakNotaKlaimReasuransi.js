...
function searchKeyword_OnKeyUp() {
    $('#SearchKeyword').keyup(function () {
        refreshGrid("#CetakNotaKlaimReasuransiGrid");
    });
}
...
function viewReport(e){
    showProgressOnGrid('#CetakNotaKlaimReasuransiGrid');
    ...
    ajaxPost("/CetakNotaKlaimReasuransi/GenerateReport", JSON.stringify(form),
        function (response) {
            if(response.Status === "OK"){
                window.open("/Reports/" + response.Data + "/CetakNotaKlaimReasuransi.pdf",  '_blank');
            } else {
                showMessage('Error', response.Message);
            }
            closeProgressOnGrid('#CetakNotaKlaimReasuransiGrid');
        }, 
    );
}