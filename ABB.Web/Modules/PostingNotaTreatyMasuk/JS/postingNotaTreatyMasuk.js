$(document).ready(function () {
    searchKeyword_OnKeyUp();
});

var selectedRowsData = [];

function searchKeyword_OnKeyUp() {
    $('#SearchKeyword').keyup(function () {
        refreshGrid("#PostingNotaTreatyMasukGrid");
    });
}

function searchFilterPostingNotaTreatyMasuk(e) {
    const gridReq = buildGridRequest(e, "SearchKeyword");

    return {
        grid: gridReq
    };
}

function onChangeGridPostingNotaTreatyMasuk(e) {
    var grid = e.sender;
    var selectedIds = grid.selectedKeyNames();

    // Clear our tracker and rebuild based on what's currently in the visible view
    // and what was already selected.
    var currentData = grid.dataSource.view();

    currentData.forEach(item => {
        var index = selectedRowsData.findIndex(x => x.id == item.id);
        var isSelected = selectedIds.includes(item.id.toString());

        if (isSelected && index === -1) {
            // If selected and not in our list, add it
            selectedRowsData.push({
                id: item.id,
                kd_cb: item.kd_cb,
                jns_tr: item.jns_tr,
                jns_nt_msk: item.jns_nt_msk,
                kd_thn: item.kd_thn,
                kd_bln: item.kd_bln,
                no_nt_msk: item.no_nt_msk,
                jns_nt_kel: item.jns_nt_kel,
                no_nt_kel: item.no_nt_kel
            });
        } else if (!isSelected && index !== -1) {
            // If deselected and in our list, remove it
            selectedRowsData.splice(index, 1);
        }
    });
}

function postingNotaTreatyMasuk(){
    showConfirmation('Confirmation', `Are you sure you want to posting?`,
        function () {
            showProgressOnGrid('#PostingNotaTreatyMasukGrid');

            ajaxPost("/PostingNotaTreatyMasuk/Posting", JSON.stringify(selectedRowsData),
                function (response) {
                    if(response.Status === "OK"){
                        showMessage("Success", "Posting Sukses")
                    } else {
                        showMessage('Error', response.Message);
                    }
                    var grid = $("#PostingNotaTreatyMasukGrid").data("kendoGrid");
                    if (grid) {
                        grid.clearSelection();
                    }
                    selectedRowsData = [];
                    refreshGrid('#PostingNotaTreatyMasukGrid');
                    closeProgressOnGrid('#PostingNotaTreatyMasukGrid');
                },
            );
        }
    );
}