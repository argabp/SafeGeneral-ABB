...
function onSearchClick() {
    ...
    $.ajax({
        url: '/LaporanNeracaSaldo117/GenerateReport',
        type: 'POST',
        contentType: 'application/json',
        data: JSON.stringify(formData),
        success: function (response) {
            if (response.Status === "OK") {
                window.open("/Reports/" + response.Data + "/LaporanNeracaSaldo117.pdf", '_blank');
            } else {
...
function onExcelClick() {
    ...
    $.ajax({
        url: '/LaporanNeracaSaldo117/GenerateExcel', // Nembak ke fungsi baru
        type: 'POST',
        contentType: 'application/json',
        data: JSON.stringify(formData),
        success: function (response) {
