...
using ABB.Application.LaporanNeracaSaldos117.Queries;
using System.Linq;
using DinkToPdf;
...
namespace ABB.Web.Modules.LaporanNeracaSaldo117
{
    public class LaporanNeracaSaldo117Controller : AuthorizedBaseController
    {
        private readonly IReportGeneratorService _reportGeneratorService;

        public LaporanNeracaSaldo117Controller(IReportGeneratorService reportGeneratorService)
        {
        ...
        {
            var filterText = text ?? "";
            var data = await Mediator.Send(new GetAllTipeAkun117Query() { SearchKeyword = filterText });
            var list = data.Select(x => new
            {
                Value = x.Kode.Trim(),
                Text = $"{x.Kode.Trim()} - {x.NamaTipe.Trim()}" 
            }).ToList();
        ...
        [HttpPost]
        public async Task<IActionResult> GenerateReport([FromBody] LaporanNeracaSaldo117FilterDto model)
        {
            try
            {
                ...
                var query = new GetLaporanNeracaSaldo117Query
                {
        ...
        [HttpPost]
        public async Task<IActionResult> GenerateExcel([FromBody] LaporanNeracaSaldo117FilterDto model)
        {
            try
            {
                var databaseName = Request.Cookies["DatabaseValue"];
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    worksheet.Cell(1, 1).Value = "LAPORAN NERACA SALDO 117";
    ...
    public class LaporanNeracaSaldo117FilterDto
    {
