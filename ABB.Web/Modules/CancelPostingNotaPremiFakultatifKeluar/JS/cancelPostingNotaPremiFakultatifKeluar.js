...
function searchKeyword_OnKeyUp() {
    $('#SearchKeyword').keyup(function () {
        refreshGrid("#CancelPostingNotaPremiFakultatifKeluarGrid");
    });
}

function searchFilterCancelPostingNotaPremiFakultatifKeluar(e) {
...
function onChangeGridCancelPostingNotaPremiFakultatifKeluar(e) {
    var grid = e.sender;
...
function accountingPostingNotaPremiFakultatifKeluar(){
    showConfirmation('Confirmation', `Are you sure you want to Posting Accounting?`,
        function () {
            showProgressOnGrid('#CancelPostingNotaPremiFakultatifKeluarGrid');

            ajaxPost("/CancelPostingNotaPremiFakultatifKeluar/PostingAccounting", JSON.stringify(selectedRowsData),
                function (response) {
                    ...
                    var grid = $("#CancelPostingNotaPremiFakultatifKeluarGrid").data("kendoGrid");
                    if (grid) {
                        grid.clearSelection();
                    }
                    selectedRowsData = [];
                    refreshGrid('#CancelPostingNotaPremiFakultatifKeluarGrid');
                    closeProgressOnGrid('#CancelPostingNotaPremiFakultatifKeluarGrid');
                },
            );
        }
    );
...
function cancelPostingNotaPremiFakultatifKeluar(){
    showConfirmation('Confirmation', `Are you sure you want to cancel?`,
        function () {
            showProgressOnGrid('#CancelPostingNotaPremiFakultatifKeluarGrid');

            ajaxPost("/CancelPostingNotaPremiFakultatifKeluar/Cancel", JSON.stringify(selectedRowsData),
                function (response) {
                    if(response.Status === "OK"){
