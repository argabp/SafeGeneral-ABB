...
function searchKeyword_OnKeyUp() {
    $('#SearchKeyword').keyup(function () {
        refreshGrid("#CetakNotaKoasuransiKeluarGrid");
    });
}
...
function viewReport(e){
    showProgressOnGrid('#CetakNotaKoasuransiKeluarGrid');
    ...
    ajaxPost("/CetakNotaKoasuransiKeluar/GenerateReport", JSON.stringify(form),
        function (response) {
            if(response.Status === "OK"){
                window.open("/Reports/" + response.Data + "/CetakNotaKoasuransiKeluar.pdf",  '_blank');
            } else {
                showMessage('Error', response.Message);
            }
            closeProgressOnGrid('#CetakNotaKoasuransiKeluarGrid');
        }, 
    );
}