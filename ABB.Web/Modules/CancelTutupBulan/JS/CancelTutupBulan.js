function executeClosing() {
    ...
    var grid = $("#GridCancelTutupBulan").data("kendoGrid");
    ...
    if (selectedData.length === 0) {
        showMessage("Info", "Silakan centang periode yang akan dicancel terlebih dahulu.");
        return;
    }

    // 5. Konfirmasi sebelum eksekusi (PENTING karena Tutup Bulan biasanya fatal kalau salah)
    showConfirmation('Konfirmasi Cancel Tutup Bulan', 
        `Apakah Anda yakin ingin melakukan Cancel Closing untuk <b>${selectedData.length}</b> periode terpilih? <br/>Proses ini tidak dapat dibatalkan.`,
        function () {
            // Tampilkan Loading di atas Grid biar user tau proses berjalan
            showProgressOnGrid('#GridCancelTutupBulan');

            // 6. Kirim ke Controller via AJAX
            // URL sesuaikan dengan Controller: /ProsesTutupBulan/ProcessClosing
            ajaxPost("/CancelTutupBulan/ProcessCancelClosing", JSON.stringify(selectedData),
                function (response) {
                    
                    // Matikan Loading
                    closeProgressOnGrid('#GridCancelTutupBulan');
