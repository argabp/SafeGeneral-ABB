...
function deleteAkseptasiResikoOtherCargo() {
    ...
    ajaxPost(`/ProsesPremiFakultatifMasuk/DeleteOtherCargo`, JSON.stringify(data), function (response) {
        if (response.Result) {
...
function btnSaveAkseptasiResikoOtherCargo_Click() {
    $('#btn-save-akseptasiResikoOtherCargo').click(function () {
        showProgress('#AkseptasiWindow');
        setTimeout(function () {
            saveAkseptasiResikoOtherCargo('/ProsesPremiFakultatifMasuk/SaveAkseptasiOtherCargo')
        }, 500);
    });
}
...
function saveAkseptasiResikoOtherCargo(url) {
    ...
    form.append("no_updt", fakultatifResiko.no_updt);
    form.append("no_rsk", fakultatifResiko.no_rsk);
    form.append("kd_endt", fakultatifResiko.kd_endt);
...
function OnKodeAlatAngkutChange(e){
    ajaxGet(`/ProsesPremiFakultatifMasuk/GenerateNamaKapal?kd_kapal=${e.sender._cascadedValue}`, (returnValue) => {
