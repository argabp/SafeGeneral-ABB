...
function btnSaveAkseptasi_Click() {
    $('#btn-save-akseptasi').click(function () {
        showProgress('#AkseptasiWindow');
        setTimeout(function () {
            saveAkseptasi('/ProsesPremiFakultatifMasuk/SaveAkseptasi')
        }, 500);
    });
}
...
function OnTglMulChange(e){
    ...
    ajaxPostSafely("/ProsesPremiFakultatifMasuk/GetTahunUnderwriting", dataJson , (returnValue) => {
...
function OnTglAkhChange(e){
    ...
    var dataJson = JSON.stringify(data);
    ajaxPostSafely("/ProsesPremiFakultatifMasuk/GetJangkaWaktuPertanggungan", dataJson , (returnValue) => {
...
function OnPolisIndukChange(e){
    ajaxGet("/ProsesPremiFakultatifMasuk/GetPolisInduk?no_pol_induk=" + e.sender._cascadedValue, (returnValue) => {
...
function OnKodeRekananTertanggungChange(e){
    ajaxGet(`/ProsesPremiFakultatifMasuk/GetKodeRekananTertanggung?kd_cb=${$("#kd_cb").val()}&kd_grp_rk=${$("#kd_grp_ttg").val()}&kd_rk=${e.sender._cascadedValue}`, (returnValue) => {
...
function OnKodeRekananSumberBisnisChange(e){
    ajaxGet(`/ProsesPremiFakultatifMasuk/GetKodeAkseptasi?st_pas=${$("#st_pas").val()}&kd_grp_sb_bis=${$("#kd_grp_sb_bis").val()}&kd_rk_sb_bis=${e.sender._cascadedValue}&no_fax="Y"`, (returnValue) => {        
...
function OnJkWktMainChange(e){
    ajaxGet(`/ProsesPremiFakultatifMasuk/GetTglAkhPtg?jk_wkt_main=${e.sender.value()}&tgl_akh_ptg=${$("#tgl_akh_ptg").val()}`, (returnValue) => {        
