...
function btnRecalculatePremi_Click() {
    $('#btnRecalculatePremi').click(function () {
        ...
        setTimeout(function () {
            ajaxGet(`/ProsesPremiFakultatifMasuk/RecalculatePremi?kd_cb=${$("#kd_cb").val()}&kd_cob=${$("#kd_cob").val()}&kd_scob=${$("#kd_scob").val()}&kd_thn=${$("#kd_thn").val()}&no_aks=${$("#no_aks").val()}&no_updt=${$("#no_updt").val()}`,
                function (response) {
                    refreshGrid("#AkseptasiPranotaGrid");
...
function btnEditAkseptasiPranota_OnClick(e) {
    ...
    openAkseptasiPranotaWindow(`/ProsesPremiFakultatifMasuk/EditPranota?kd_cb=${dataItem.kd_cb}&kd_cob=${dataItem.kd_cob}&kd_scob=${dataItem.kd_scob}&kd_thn=${dataItem.kd_thn}&no_aks=${dataItem.no_aks}&no_updt=${dataItem.no_updt}&kd_mtu=${dataItem.kd_mtu}`, 'Edit Pranota');
}
...
function deleteAkseptasiPranota(dataItem) {
    ajaxGet(`/ProsesPremiFakultatifMasuk/DeletePranota?kd_cb=${dataItem.kd_cb}&kd_cob=${dataItem.kd_cob}&kd_scob=${dataItem.kd_scob}&kd_thn=${dataItem.kd_thn}&no_aks=${dataItem.no_aks}&no_updt=${dataItem.no_updt}&kd_mtu=${dataItem.kd_mtu}`, function (response) {
        if (response.Result) {
...
function OnPranotaChange(e){
    var grid = e.sender;
    fakultatifPranota = grid.dataItem(this.select());
