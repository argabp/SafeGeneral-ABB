...
function deleteAkseptasiObyekCIS(dataItem) {
    ajaxGet(`/ProsesPremiFakultatifMasuk/DeleteObyekCIS?kd_cb=${dataItem.kd_cb}&kd_cob=${dataItem.kd_cob}&kd_scob=${dataItem.kd_scob}&kd_thn=${dataItem.kd_thn}&no_aks=${dataItem.no_aks}&no_updt=${dataItem.no_updt}&no_rsk=${dataItem.no_rsk}&kd_endt=${dataItem.kd_endt}&no_oby=${dataItem.no_oby}`, function (response) {
        if (response.Result) {
...
function saveAkseptasiObyekCIS(dataItem) {   
    ...
    ajaxPost("/ProsesPremiFakultatifMasuk/SaveAkseptasiObyekCIS", JSON.stringify(data),
        function (response) {
            refreshGrid("#AkseptasiObyekGrid");
