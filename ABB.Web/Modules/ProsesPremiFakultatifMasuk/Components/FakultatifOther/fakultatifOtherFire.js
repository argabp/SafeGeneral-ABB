...
function deleteAkseptasiResikoOtherFire() {
    ...
    ajaxPost(`/ProsesPremiFakultatifMasuk/DeleteOtherFire`, JSON.stringify(data), function (response) {
        if (response.Result) {
...
function btnSaveAkseptasiResikoOther_Click() {
    $('#btn-save-akseptasiResikoOtherFire').click(function () {
        showProgress('#AkseptasiWindow');
        setTimeout(function () {
            saveAkseptasiResikoOther('/ProsesPremiFakultatifMasuk/SaveAkseptasiOtherFire')
        }, 500);
    });
}
...
function btnOpenLokasResiko() {
    $('#lokasi-resiko').click(function () {
        openWindow('#LokasiResikoWindow', "/ProsesPremiFakultatifMasuk/LokasiResiko", "Lokasi Resiko");
    });
}
...
function onLokasiResikoChange(e){
    ajaxGet("/ProsesPremiFakultatifMasuk/GetLokasiResikoDetail?kd_lok_rsk=" + e.sender._value, (returnValue) => {
...
function onKodeOkpasiChange(e){
    ajaxGet(`/ProsesPremiFakultatifMasuk/GetKodeOkupasiDetail?kd_zona=${$("#kd_zona").val()}&kd_kls_konst=${$("#kd_kls_konst").val()}&kd_okup=${e.sender._cascadedValue}&kd_scob=${$("#kd_scob").val()}`, (returnValue) => {
