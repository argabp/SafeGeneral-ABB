...
function btnSaveAkseptasiResikoOther_Click() {
    $('#btn-save-akseptasiResikoOtherHull').click(function () {
        showProgress('#AkseptasiWindow');
        setTimeout(function () {
            saveAkseptasiResikoOther('/ProsesPremiFakultatifMasuk/SaveAkseptasiOtherHull')
        }, 500);
    });
}
...
function OnKodeKapalChange(e){
    ajaxGet(`/ProsesPremiFakultatifMasuk/GenerateDataKapal?kd_kapal=${e.sender.value()}`, (returnValue) => {
