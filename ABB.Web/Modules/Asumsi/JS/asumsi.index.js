...
function onSaveAsumsiPeriode(dataItem){
    ...
    ajaxPost("/Asumsi/AddAsumsiPeriode", JSON.stringify(data),
        function (response) {
            refreshGrid('#grid_Periode_' + dataItem.KodeAsumsi);
            if (response.Result == "OK") {
                showMessage('Success', response.Message);
            } else
                showMessage('Error', response.Message);
...
function onDeleteAsumsiPeriode(e){
    ...
    showConfirmation('Confirmation', `Are you sure you want to delete?`,
        function () {
            showProgressOnGrid('#grid_Periode_' + dataItem.KodeAsumsi);
            ...
            ajaxPost("/Asumsi/DeleteAsumsiPeriode", JSON.stringify(data),
                function (response) {
                    ...
                    refreshGrid('#grid_Periode_' + dataItem.KodeAsumsi);
                    refreshGrid('#grid_Detail' + dataItem.KodeAsumsi);
                    closeProgressOnGrid('#grid_Periode_' + dataItem.KodeAsumsi);
                }
            );
        }
    );
...
function onDeleteAsumsiDetail(e){
    ...
    showConfirmation('Confirmation', `Are you sure you want to delete?`,
        function () {
            showProgressOnGrid('#grid_Detail' + dataItem.KodeAsumsi);
            ...
            ajaxPost("/Asumsi/DeleteAsumsiDetail", JSON.stringify(data),
                function (response) {
                    ...
                    refreshGrid('#grid_Detail' + dataItem.KodeAsumsi);
                    closeProgressOnGrid('#grid_Detail' + dataItem.KodeAsumsi);
                }
            );
        }
    );
