...
function searchKeyword_OnKeyUp() {
    $('#SearchKeyword').keyup(function () {
        refreshGrid("#CetakSlipKomisiFakultatifKeluarGrid");
    });
}

function viewReport(e){
    showProgressOnGrid('#CetakSlipKomisiFakultatifKeluarGrid');
    ...
    ajaxPost("/CetakSlipKomisiFakultatifKeluar/GenerateReport", JSON.stringify(form),
        function (response) {
            if(response.Status === "OK"){
                window.open("/Reports/" + response.Data + "/CetakNotaTreatyXOLMasuk.pdf",  '_blank');
            } else {
                showMessage('Error', response.Message);
            }
            closeProgressOnGrid('#CetakSlipKomisiFakultatifKeluarGrid');
        }, 
    );
}