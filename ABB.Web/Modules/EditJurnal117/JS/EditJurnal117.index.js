...
function initDetailGridEdit(noBukti) {
    ...
    $("#EditDetailGrid").kendoGrid({
        dataSource: {
            transport: {
                read: { url: `/EditJurnal117/GetDetailJurnal?noBukti=${cleanNoBukti}`, type: "GET", dataType: "json" }
            },
            schema: {
                data: "Data", 
                model: { id: "Id" } 
...
function btnSimpanDetail_OnClick() {
    ...
    if (mataUang === "001") {
        ...
        $.ajax({
            url: '/EditJurnal117/GetKurs', type: 'GET', data: { kodeMataUang: mataUang, tanggalVoucher: tglBukti },
            success: function(res) {
        ...
        $("#Detail_NilaiOrg").kendoNumericTextBox({ 
            ...
            change: function() {
                ...
                if (mataUang === "001") {
                    ...
                    $.ajax({
                        url: '/EditJurnal117/GetKurs', type: 'GET',
    ...
    if (!$("#Detail_KodeAkun").data("kendoComboBox")) {
        $("#Detail_KodeAkun").kendoComboBox({
            ...
            dataSource: {
                ...
                transport: {
                    read: {
                        url: "/EditJurnal117/GetCoa", type: "GET", dataType: "json",
...
function btnCari_OnClick() {
    $("#EditJurnal117Grid").data("kendoGrid").dataSource.read();
}
...
function btnEditJurnal_OnClick(e) {
    e.preventDefault();
    var grid = $("#EditJurnal117Grid").data("kendoGrid");
    var row = $(e.target).closest("tr");
    var dataItem = grid.dataItem(row);
    var window = $("#EditJurnal117Window").data("kendoWindow");
    ...
    window.refresh({ url: `/EditJurnal117/Edit?noBukti=${cleanNoBukti}` });
    window.center().open();
}

function btnCloseWindow() {
    $("#EditJurnal117Window").data("kendoWindow").close();
}
function initDetailGridEdit(noBukti) {
    var cleanNoBukti = encodeURIComponent(noBukti.trim());
    ...
    $("#EditDetailGrid").kendoGrid({
        dataSource: {
            transport: {
                read: {
                    url: `/EditJurnal117/GetDetailJurnal?noBukti=${cleanNoBukti}`,
                    type: "GET",
                    dataType: "json"
                }
            },
                    ...
                    $('<input required name="' + options.field + '"/>')
                        ...
                        .kendoComboBox({
                            ...
                            dataSource: {
                                ...
                                transport: {
                                    read: {
                                        // PASTIKAN URL-NYA SUDAH MENGARAH KE EDIT JURNAL 117
                                        url: "/EditJurnal117/GetCoa", 
                                        type: "GET",
                                        dataType: "json",
                                        data: function(options) {
...
function updateFooterTotalsEdit() {
    ...
    // Perbaikan: Bulatkan ke 2 desimal untuk mencegah masalah floating-point JS
    totalDebet = Math.round(totalDebet * 100) / 100;
    totalKredit = Math.round(totalKredit * 100) / 100;
    
    var balance = Math.round((totalDebet - totalKredit) * 100) / 100;
    ...
    // Perbaikan: Pastikan balance benar-benar 0, bukan kurang dari 0.01
    if (balance === 0) {
        lblBalance.css("color", "green").text("0.00 (Balance)");
    } else {
        lblBalance.css("color", "red").text(kendo.toString(balance, "n2"));
    }
}
...
function onSaveAllEditJurnal() {
    ...
    debet = Math.round(debet * 100) / 100;
    kredit = Math.round(kredit * 100) / 100;

    
    // Pastikan debet dan kredit harus benar-benar sama
    if (debet !== kredit) {
    ...
    if (confirm("Pastikan nominal debet & kredit sudah sesuai. Lanjutkan simpan perubahan?")) {
        $.ajax({
            type: "POST",
            url: "/EditJurnal117/SaveEditJurnal",
