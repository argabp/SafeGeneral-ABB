...
function searchKeyword_OnKeyUp() {
    $('#SearchKeyword').keyup(function () {
        refreshGrid("#CetakNotaKlaimGrid");
    });
}
...
function viewReport(e){
    showProgressOnGrid('#CetakNotaKlaimGrid');
    ...
    ajaxPost("/CetakNotaKlaim/GenerateReport", JSON.stringify(form),
        function (response) {
            if(response.Status === "OK"){
                window.open("/Reports/" + response.Data + "/CetakNotaKlaim.pdf",  '_blank');
            } else {
                showMessage('Error', response.Message);
            }
            closeProgressOnGrid('#CetakNotaKlaimGrid');
        }, 
    );
}