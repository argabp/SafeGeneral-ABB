...
function searchKeyword_OnKeyUp() {
    $('#SearchKeyword').keyup(function () {
        refreshGrid("#CetakDLAReasuransiGrid");
    });
}
...
function viewReport(element){
    showProgressOnGrid('#CetakDLAReasuransiGrid');
    
    var id = $(element).parent().parent().parent()[0].id;
    var dataItem = $("#CetakDLAReasuransiGrid").getKendoGrid().dataItem($(element).parent().parent().parent().parent().parent()[0].previousElementSibling);
    ...
    ajaxPost("/CetakDLAReasuransi/GenerateReport", JSON.stringify(form),
        function (response) {
            if(response.Status === "OK"){
                window.open("/Reports/" + response.Data + "/CetakDLAReasuransi.pdf",  '_blank');
            } else {
                showMessage('Error', response.Message);
            }
            closeProgressOnGrid('#CetakDLAReasuransiGrid');
        }, 
    );
}