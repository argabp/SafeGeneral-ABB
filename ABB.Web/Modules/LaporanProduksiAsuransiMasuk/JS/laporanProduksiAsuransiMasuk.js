...
function btnPreview_Click() {
    $('#btn-preview').click(function () {
        showProgressByElement($('#LaporanProduksiAsuransiMasukForm'));
        setTimeout(function () {
            previewReport('/LaporanProduksiAsuransiMasuk/GenerateReport')
        }, 500);
    });
}

function previewReport(url) {
    var form = getFormData($('#LaporanProduksiAsuransiMasukForm'));
    ...
    ajaxPost(url, data,
        function (response) {
            if(response.Status === "OK"){
                window.open("/Reports/" + response.Data + "/LaporanProduksiAsuransiMasuk.pdf",  '_blank');
            } else {
                showMessage('Error', response.Message);
            }

            closeProgressByElement($('#LaporanProduksiAsuransiMasukForm'));
        }
    );
}
...
function OnKodeRekananTertanggungChange(e){
    ajaxGet(`/LaporanProduksiAsuransiMasuk/GetKodeRekananTertanggung?kd_cb=${$("#kd_cb").val()}&kd_grp_rk=${$("#kd_grp_ttg").val()}&kd_rk=${e.sender._cascadedValue}`, (returnValue) => {
