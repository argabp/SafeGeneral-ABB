...
function btnSaveEntriNota_Click() {
    $('#btn-save-entriNota').click(function () {
        showProgress('#EntriNotaWindow');
        setTimeout(function () {
            saveEntriNota('/NotaPremiFakultatifMasuk/SaveEntriNota')
        }, 500);
    });
}
...
function btnCancelEntriNota_Click() {
    $('#btn-cancel-entriNota').click(function () {
        openWindow("#EntriNotaCancelWindow", "/NotaPremiFakultatifMasuk/NotaCancel", "Nota Cancel")
    });
}
...
function saveEntriNota(url){
    ...
    ajaxGet(`/NotaPremiFakultatifMasuk/ValidateSaveDetailNota?no_pol=${$("#no_pol").val()}&nilai_nt=${$("#nilai_nt").val()}&nilai_ang=${totalAng}`, 
