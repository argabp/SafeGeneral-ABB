...
function searchKeyword_OnKeyUp() {
    $('#SearchKeyword').keyup(function () {
        refreshGrid("#CancelUpdateSettledKlaimGrid");
    });
}

function cancelUpdateSettledKlaim(){
    showConfirmation('Confirmation', `Are you sure you want to update?`,
        function () {
            showProgressOnGrid('#CancelUpdateSettledKlaimGrid');
            // Get the Kendo UI Grid instance
            var grid = $("#CancelUpdateSettledKlaimGrid").data("kendoGrid");
            ...
            ajaxPost("/CancelUpdateSettledKlaim/CancelUpdate", JSON.stringify(selectedData),
                function (response) {
                    if(response.Status === "OK"){
                        showMessage("Success", "Cancel Update Sukses")
                    } else {
                        showMessage('Error', response.Message);
                    }
                    refreshGrid('#CancelUpdateSettledKlaimGrid');
                    closeProgressOnGrid('#CancelUpdateSettledKlaimGrid');
                },
            );
        }
    );
