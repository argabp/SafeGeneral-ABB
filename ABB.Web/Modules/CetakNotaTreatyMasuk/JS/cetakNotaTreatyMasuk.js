...
function searchKeyword_OnKeyUp() {
    $('#SearchKeyword').keyup(function () {
        refreshGrid("#CetakNotaTreatyMasukGrid");
    });
}

function viewReport(e){
    showProgressOnGrid('#CetakNotaTreatyMasukGrid');
    ...
    ajaxPost("/CetakNotaTreatyMasuk/GenerateReport", JSON.stringify(form),
        function (response) {
            if(response.Status === "OK"){
                window.open("/Reports/" + response.Data + "/CetakNotaTreatyMasuk.pdf",  '_blank');
            } else {
                showMessage('Error', response.Message);
            }
            closeProgressOnGrid('#CetakNotaTreatyMasukGrid');
        }, 
    );
}