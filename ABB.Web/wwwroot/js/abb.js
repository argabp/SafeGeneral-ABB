...
function brTemplateComma(value) {
  if (value == null) return "";
  return value.replaceAll(",", "<br />");
}
...
function numericTextBoxEditor(args) {
  var changeEvt = emptyFunction;
  var format = "#";
  var min = 0;
  var max = 0;
  var restrictDecimals = false;
  var decimals = true;
  var spinners = true;
  
  if (args.change != undefined) changeEvt = args.change;
  if (args.format != undefined) format = args.format;
  if (args.min != undefined) min = args.min;
  if (args.max != undefined) max = args.max;
  if (args.restrictDecimals != undefined) restrictDecimals = args.restrictDecimals;
  if (args.decimals != undefined) decimals = args.decimals;
  if (args.spinners != undefined) spinners = args.spinners;
