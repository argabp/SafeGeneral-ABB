...
function onAsumsiChange(e){
    var grid = e.sender;
    asumsi = grid.dataItem(this.select());
    refreshGrid("#AsumsiPeriodeGrid");
    asumsiPeriode = undefined;
    refreshGrid("#AsumsiDetailGrid");
}

function getAsumsiPeriodeParam(){
    return {
        kodeAsumsi: asumsi === undefined ? null : asumsi.KodeAsumsi,
    }
}
...
function onEditAsumsiPeriode(dataItem){
    if(dataItem.model.isNew())
        dataItem.model.KodeAsumsi = asumsi.KodeAsumsi;
}
