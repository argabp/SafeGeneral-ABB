...
function onSaveObligee(){    
    var url = "/Obligee/SaveObligee";
    ...
    showProgress('#ObligeeWindow');
    
    ajaxPost(url, JSON.stringify(data),
        function (response) {
            refreshGrid("#ObligeeGrid");
            if (response.Result == "OK") {
                showMessage('Success', response.Message);
                closeWindow("#ObligeeWindow");
            }
            else if (response.Result == "ERROR")
                showMessage('Error', response.Message);
            else
                $("#ObligeeWindow").html(response);

            closeProgress('#ObligeeWindow');
        }
    );
}

function onDeleteObligee(e){
    e.preventDefault();
    var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
    showConfirmation('Confirmation', `Are you sure you want to delete?`,
        function () {
            showProgressOnGrid('#ObligeeGrid');
            ...
            ajaxPost("/Obligee/DeleteObligee", JSON.stringify(data),
                function (response) {
                    if (response.Result == "OK") {
                        showMessage('Success', response.Message);
                    } else
                    ...
                    refreshGrid("#ObligeeGrid");
                    closeProgressOnGrid('#ObligeeGrid');
                }
            );
        }
    );
...
function onSaveDetailObligeeIndividu(){
    var url = "/Obligee/SaveDetailObligee";
    ...
    showProgress('#DetailObligeeWindow');
    ...
    ajaxPost(url, JSON.stringify(data),
        function (response) {
            ...
            closeWindow("#DetailObligeeWindow");
        }
    );
}

function onDeleteDetailObligee(e){
    e.preventDefault();
    var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
    showConfirmation('Confirmation', `Are you sure you want to delete?`,
        function () {
            ...
            ajaxPost("/Obligee/DeleteDetailObligee", JSON.stringify(data),
                function (response) {
                    if (response.Result == "OK") {
                        showMessage('Success', response.Message);
                    } else
...
function btnAddObligee_OnClick() {
    openWindow('#ObligeeWindow', `/Obligee/AddObligeeView`, 'Add');
}

function btnEditObligee_OnClick(e) {
    e.preventDefault();
    var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
    console.log('dataItem', dataItem);
    openWindow('#ObligeeWindow', `/Obligee/EditObligeeView?kd_cb=${dataItem.kd_cb.trim()}&kd_grp_rk=${dataItem.kd_grp_rk}&kd_rk=${dataItem.kd_rk}`, 'Edit');
}

function btnEditDetailObligee_OnClick(e) {
    e.preventDefault();
    var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
    selectedData = dataItem;
    console.log('dataItem', dataItem);
    openWindow('#DetailObligeeWindow', `/Obligee/EditDetailObligeeView?kd_cb=${dataItem.kd_cb.trim()}&kd_grp_rk=${dataItem.kd_grp_rk}&kd_rk=${dataItem.kd_rk}&flag_sic=${dataItem.flag_sic}`, 'Edit');
}